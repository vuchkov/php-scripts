<?php
// Online visitors counter script.
//session_start();

// Add: include("visitors.php") in the footer of each page.
...
function CountVisitors() {
    ...
    if(is_array($dbary)) {
        foreach($dbary as list($user_ip, $user_session, $user_time)) {
            if(($user_ip != $cur_ip) && (($user_time + $expire) > $cur_time)) {
                $dbary_new[$user_ip] = [$user_session, $user_time];
            }
        }
    }
    $dbary_new[$cur_ip] = [$cur_session, $cur_time]; // add record for current user
...
/*
  ...
  * https://coursesweb.net/php-mysql/register-show-online-users-visitors_t
  */
